...
const filenames = ['README.md', 'notes/离线与隐私.md', 'notes/图表与时序.md', 'preview.html', 'styles/preview.css', 'images/md-preview.png'];
...
try {
  ...
  await page.screenshot({ path: path.join(output, '01-reading-1280x800.png'), animations: 'disabled' });
  await page.getByRole('tab', { name: '文件', exact: true }).click();
  await page.getByRole('button', { name: 'notes', exact: true }).click();
  await expect(page.getByRole('button', { name: '图表与时序.md', exact: true })).toBeVisible();
  await page.screenshot({ path: path.join(output, '02-files-1280x800.png'), animations: 'disabled' });
  ...
  await page.screenshot({ path: path.join(output, '03-mermaid-1280x800.png'), animations: 'disabled' });
  ...
  await page.screenshot({ path: path.join(output, '04-dark-1280x800.png'), animations: 'disabled' });
  ...
  await page.getByRole('tab', { name: /^大纲/ }).click();
  await page.screenshot({ path: path.join(output, '05-html-1280x800.png'), animations: 'disabled' });
